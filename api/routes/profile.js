...
profile.post("/nameupdate", (req, res) => {
  ...
  if (!first_name || !last_name || !user_id) {
    return res.status(400).send({ message: "First name,last name and userid is required" });
  }
...
profile.post("/emailupdate", (req, res) => {
  ...
  if (!email || !user_id) {
    return res.status(400).send({ message: "Email and userid required" });
  }
...
profile.post("/passwordupdate", async (req, res) => {
  ...
  if (!old_password || !new_password || !user_id)
    return res.status(400).send({ message: "Both old and new password and userid required" });
  const getOldPasswordQuery = "SELECT password FROM user WHERE id=?;";
  db.query(getOldPasswordQuery, [user_id], async (error, result) => {
    if (error)
      return res.status(500).send({ error, message: "Database error during password fetch" });
    if (result.length === 0) {
      return res.status(400).send({ message: "User with this id does not exists" });
    }
    ...
    if (isMatch) {
      ...
      db.query(updateQuery, [newPaswordHash, user_id], (error, result) => {
        if (error)
          return res.status(500).send({ error, message: "Database error during password update" });
        res.send({ message: "Password update successfull" });
      });
    } else {
      return res.status(400).send({ message: "Your old password does not match" });
    }
  });
});
