...
const myexams = express.Router();

myexams.post("/short/get-exams", verfiyToken, async (req, res) => {
  const { user_id } = req;
  const connection = await db.getConnection();
  try {
    const getExamsQuery =
      "SELECT title, e.id, duration_min, scheduled_date, exam_code, no_of_attempts, created_at, exam_type, domain_name, field_name FROM exam AS e JOIN domain AS d ON e.domain_id=d.id JOIN field ON d.field_id=field.id WHERE user_id=? ORDER BY created_at DESC";
    let [exams] = await connection.query(getExamsQuery, [user_id]);

    for (let i = 0; i < exams.length; i++) {
      const statsQuery =
        "SELECT COUNT(*) AS total_attempts, MAX(awarded_marks) AS max, AVG(awarded_marks) AS avg, MIN(awarded_marks) AS min FROM exam_attempt WHERE exam_id=?";
      ...
      const [total_marks] = await connection.query(totalMarksQuery, [
        exams[i].id,
      ]);
      exams[i].total_marks = total_marks[0].total_marks;
      exams[i].max = stats[0].max;
      exams[i].avg = stats[0].avg;
      exams[i].min = stats[0].min;
    }
    const getAttemptsQuery =
      "SELECT a.id AS id, submitted_at, awarded_marks, exam_id, title, scheduled_date, exam_type, restriction_level, duration_min, exam_code, evaluation, domain_name, field_name FROM exam_attempt AS a JOIN exam AS e ON a.exam_id = e.id JOIN domain AS d ON e.domain_id = d.id JOIN field ON d.field_id=field.id WHERE a.user_id=? ORDER BY submitted_at DESC";
