...
//     <div
//       className={cn(
//         "h-full w-full rounded-lg border bg-background p-6 shadow-lg transition-all duration-300 ease-in-out",
//         className
//       )}
...
export const DialogStackNext = ({
  ...
  const handleNext = () => {
    if (context.activeIndex < context.totalDialogs - 1) {
      const current = context.activeIndex;
      // trigger fade-out animation
      const dialog = document.querySelector(
        `[data-dialog-index="${current}"]`
      ) as HTMLElement;
      if (dialog) dialog.classList.add("opacity-0", "translate-y-4");

      setTimeout(() => {
        context.setActiveIndex(current + 1);
      }, 250); // wait for 250ms transition
    }
  };
